...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/Johannes/Documents/phd/psychopy tutorial/RL_task_clickable/simple_RL_lastrun.py',
...
for thisLoopTraining in loopTraining:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if pic_cue.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > pic_cue.tStartRefresh + 3-frameTolerance:
                # keep track of stop time/frame for later
                pic_cue.tStop = t  # not accounting for scr refresh
                pic_cue.frameNStop = frameN  # exact frame index
                win.timeOnFlip(pic_cue, 'tStopRefresh')  # time at next scr refresh
                pic_cue.setAutoDraw(False)
        
        # *pic_left* updates
        if pic_left.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
            # keep track of start time/frame for later
        ...
        if pic_left.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > pic_left.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                pic_left.tStop = t  # not accounting for scr refresh
                pic_left.frameNStop = frameN  # exact frame index
                win.timeOnFlip(pic_left, 'tStopRefresh')  # time at next scr refresh
                pic_left.setAutoDraw(False)
        
        # *pic_right* updates
        if pic_right.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
            # keep track of start time/frame for later
        ...
        if pic_right.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > pic_right.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                pic_right.tStop = t  # not accounting for scr refresh
                pic_right.frameNStop = frameN  # exact frame index
                win.timeOnFlip(pic_right, 'tStopRefresh')  # time at next scr refresh
                pic_right.setAutoDraw(False)
        # *key* updates
        if key.status == NOT_STARTED and t >= 1-frameTolerance:
            # keep track of start time/frame for later
